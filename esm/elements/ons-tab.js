...
template.innerHTML = `
  ...
  <input id="input" type="radio" style="display: none" />
  <button id="button" class="tabbar__button">
    <ons-ripple id="ripple" disabled></ons-ripple>
    <div class="tabbar__icon">
      <slot name="_icon"></slot>
    </div>
    <div id="label" class="tabbar__label"></div>
    <div id="badge" class="tabbar__badge notification"></div>
  </button>
`;
...
export default class TabElement extends BaseElement {
  ...
  static get observedAttributes() {
    return ['label', 'badge', 'icon', 'active-icon', 'active', 'modifier', 'ripple'
  //'class'
    ];
  }
  ...
  set ripple(value) {
    this.toggleAttribute('ripple', value);
  }
  ...
  _defineElements() {
    ...
    this._badgeElement = this.shadowRoot.querySelector('#badge');
    this._rippleElement = this.shadowRoot.querySelector('#ripple');
  }

  _requireLightDomIcon() {
    if (!this._iconElement) {
      this._iconElement = document.createElement('ons-icon');
      this._iconElement.setAttribute('slot', '_icon');
      this.appendChild(this._iconElement);
    }
  }

  _upgradeProperties() {
    ['page', 'icon', 'activeIcon', 'label', 'badge', 'active', 'ripple']
      .forEach(property => {
  ...
  _applyAutoStyling() {
    // UH OH!
    // Autostyling is broken in some parts.
    // For example, ripple is not working correctly now that we create an
    // ons-ripple[disabled] in the ons-tab template.
  ...
  _updateRippleElement() {
    this._rippleElement.disabled = !this.ripple;
  }
...
:checked + .tabbar--top-border__button {
  background-color: transparent;
  border-bottom: 4px solid rgba(0, 118, 255);
